 ...
 * Created by xiyuanbupt on 9/12/16.
 A sequence of numbers is called a wiggle
 sequence if the differences between successive
 ...
 the first because its first two differences
 ...
 Output: 7
 There are several subsequences that
...
public class Solution_376 {
    ...
    public int wiggleMaxLength(int[] nums) {
        ...
        boolean incr = (nums[k] - nums[0]) > 0 ;
        int res = 1;
        nums[res++] = nums[k++];
        incr = !incr;
        while (k<nums.length){
            if(incr&&nums[k]>nums[k-1]){
                nums[res++] = nums[k];
                incr = !incr;
            }else if(!incr&&nums[k]<nums[k-1]){
                nums[res++] = nums[k];
                incr = !incr;
            }
            k++;
    ...
    public static void main(String[] args){
        Solution_376 sl = new Solution_376();
